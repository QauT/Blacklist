...
// بأختصار شديد جدا الكود كامل انا مبرمجه ..
// Smartu ON TOP ..
// Developers Area ON TOP ..
// https://discord.gg/8cWWjnJngp

const mongoose = require("mongoose");
const User = require("./Models/User");
const Guild = require("./Models/Guild");

mongoose.connect("mongodb://127.0.0.1:27017/BlackListSRC").then(() => {
  ...
  client.on("messageCreate", async (message) => {
    ...
    if (command === "search") {
      ...
      .setFooter('Made By Kevin.#0007', message.author.displayAvatarURL({ dynamic: true }));
      let more = null;
    ...
    if (command === "blacklist") {
      ...
      if (args[0] == "add") {
        ...
        collector.on("collect",async msg => {
          ...
          embed.setFooter('Made By Kevin.#0007', message.author.displayAvatarURL({ dynamic: true }));
  ...
  client.on("guildMemberAdd", async member => {
    ...
    member.ban({
      reason: `Blacklisted By the bot`
    }).catch(_=> {
      member.guild.fetchOwner().then(owner => {
        owner.send(`I can't ban ${member} even this member was blacklisted by the bot, please make sure i have BAN_MEMBERS permission in your server`);
      });
